...
flags.DEFINE_boolean(
    ...
    "True if we want to compute the simulate the convergence map")
flags.DEFINE_boolean("Power_Spectrum", False,
                     "True if we want to compute the jacobian of the ps")
flags.DEFINE_boolean("Peak_counts", False,
                     "True if we want to compute the jacobian of the pk")
flags.DEFINE_boolean("l1_norm", False,
                     "True if we want to compute the jacobian of the l1norm")
...
@tf.function
def compute_jacobian_ps(Omega_c, sigma8, Omega_b, n_s, h, w0, Aia):
    ...
    with tf.GradientTape() as tape:
        tape.watch(params)
        m = compute_kappa(params[0], params[1], params[2], params[3],
...
def compute_jacobian_l1norm(Omega_c, sigma8, Omega_b, n_s, h, w0, Aia):
    ...
    with tf.GradientTape() as tape:
        tape.watch(params)
        kmap_IA = compute_kappa(params[0], params[1], params[2], params[3],
                                params[4], params[5], params[6])
...
def main(_):
    global ode_function
    ode_function = make_neural_ode_fn(FLAGS.nc, FLAGS.batch_size,
                                      FLAGS.correction_params)
