...
flags.DEFINE_integer("nc", 128, "Number of transverse voxels in the simulation volume")
flags.DEFINE_integer("field_npix", 512, "Number of pixels in the lensing field")
flags.DEFINE_float("box_size", 100., "Transverse comoving size of the simulation volume")
...
def compute_kappa(Omega_c,sigma8):
    ...
    for i,j in zip(range(len(a_center)),cycle(range(6))):
        plane = flowpm.raytracing.density_plane(states[::-1][i][1],
                                            [FLAGS.nc, FLAGS.nc, FLAGS.nc],
                                            FLAGS.nc//2, #r_center[i]/box_size*nc,
                                            matrix[j],
    ...
    xgrid, ygrid = np.meshgrid(np.linspace(0, FLAGS.field_size, FLAGS.field_npix, endpoint=False), # range of X coordinates
                           np.linspace(0, FLAGS.field_size, FLAGS.field_npix, endpoint=False)) # range of Y coordinates
...
def compute_jacobian(Omega_c, sigma8):
    ...
    with tf.GradientTape() as tape:
        ...
        ell, power_spectrum= DHOS.statistics.power_spectrum(m[0,:,:,-1],FLAGS.field_size,
                                                         FLAGS.field_npix)
...
def main(_):
  # Query the jacobian
  m,lensplanes, r_center, a_center, jacobian, ell, ps = compute_jacobian(tf.convert_to_tensor(FLAGS.Omega_c,
                                                            dtype=tf.float32),
                                       tf.convert_to_tensor(FLAGS.sigma8,
                                                            dtype=tf.float32))
  # Saving results in requested filename
  pickle.dump({'a': a_center, 'lensplanes': lensplanes, 'r': r_center, 'map': m.numpy(), 'ell': ell.numpy(),
