...
def power_spectrum(field, kmin=5, dk=0.5, boxsize=False):
  ...
  """
    ...
    dig, Nsum, xsum, W, k, kedges = _initialize_pk(shape[1:], boxsize, kmin,
                                                   dk)
    ...
    def fn(x):
        ...
        Psum = tf.dtypes.cast(tf.math.bincount(
            dig, weights=(W.flatten() * imag), minlength=xsum.size),
                              dtype=tf.complex64) * 1j
        Psum += tf.dtypes.cast(tf.math.bincount(dig,
                                                weights=(W.flatten() * real),
                                                minlength=xsum.size),
                               dtype=tf.complex64)
        return tf.dtypes.cast((Psum / Nsum)[1:-1] * boxsize.prod(),
                              dtype=tf.float32)
