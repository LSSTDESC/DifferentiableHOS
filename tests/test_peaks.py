...
def test_peak():
    ...
    DHOS_output = DifferentiableHOS.statistics.find_peaks2d_tf(tf.constant(test_map,dtype=tf.float32))
    ...
    DHOS_output = DifferentiableHOS.statistics.peaks_histogram_tf(tf.constant(test_map,dtype=tf.float32))
